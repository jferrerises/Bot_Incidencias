...
def get_issues_redmine():
    try:
        ...
        fecha1=datetime.now().strftime("2023-03-01")
...
def filter_report():
    try:
        ...
        hr = datetime.now().strftime("%Y-%m-%d")
        #fl=os.path.abspath(os.path.join('filtro',f"{config.get('AppConfig','folder_reportes')}\\INCIDENCIAS_PROCESADAS_{hr}.xlsx"))
        fl=os.path.join(root_folder,f"{config.get('AppConfig','folder_reportes')}\\INCIDENCIAS_PROCESADAS_{hr}.xlsx")
